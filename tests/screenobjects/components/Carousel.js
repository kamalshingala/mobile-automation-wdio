import Gestures from '../../helpers/utilities/Gestures';
...
class Carousel extends Gestures {
  ...
  verifyNthCardContainsText(nthCard, partialText) {
    ...
    const cardNumber =
      nthCard === 'first' || cards.length === 1 ? 0 : 1;
    ...
    return expectedText
      .split(' ')
      .forEach(word =>
        assert.include(cardText, word, 'Message is not included.'),
      );
  }
